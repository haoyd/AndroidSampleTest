...
public class StatusBarUtil {
     ...
     * @param alpha
     */
    public static void setTranslucent(Activity activity, int color, float alpha) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setTransparent(activity);
        addTranslucentView(activity, color, alpha);
    }
    ...
    /**
     * 添加半透明矩形条， 以alpha值作为参数
     * @param activity
     * @param color
     * @param statusBarAlpha
     */
    private static void addTranslucentView(Activity activity, int color, float statusBarAlpha) {
        ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
        View fakeTranslucentView = contentView.findViewById(FAKE_TRANSLUCENT_VIEW_ID);
        if (fakeTranslucentView != null) {
            if (fakeTranslucentView.getVisibility() == View.GONE) {
            ...
            fakeTranslucentView.setAlpha(statusBarAlpha);
        } else {
    ...
    /**
     * 创建半透明矩形View，以alpha值作为参数，取值范围是0到1
     * @param activity
     * @param alpha
     * @return
     */
    private static View createTranslucentStatusBarView(Activity activity, int color, float alpha) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
            new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
